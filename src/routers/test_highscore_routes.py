...
def test_get_highscores(client, db, seed_db):
    response = client.get(f"{settings.route_prefix}/highscores?score_type=single")
    print(response.request.url)
    print(client.base_url)
    assert response.status_code == 200
    assert len(response.json()) == 3
    response = client.get(f"{settings.route_prefix}/highscores?score_type=multi")
    assert response.status_code == 200
    assert len(response.json()) == 1
    response = client.get(f"{settings.route_prefix}/highscores?score_type=single&count=2")
    assert response.status_code == 200
    assert len(response.json()) == 2
    response = client.get(f"{settings.route_prefix}/highscores?score_type=single&userid=Mickey%20Public%20ID")
    assert response.status_code == 200
    assert len(response.json()) == 1
...
def test_post_highscore(client, db, seed_db, valid_jwt):
    ...
    response = client.post(f"{settings.route_prefix}/highscores",
                           headers=headers,
...
def test_post_highscore_no_such_user(client, db, seed_db, no_user_jwt):
    response = client.post(f"{settings.route_prefix}/highscores?token={no_user_jwt}",
                           json={'score_type': 'single', 'value': 75})
